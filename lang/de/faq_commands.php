...
return [
    ...
    'status_line_note' => 'Die Sticky-Statuszeile gilt für JEDEN Aufruf des Skills — nicht nur für den Auto-Modus, und ausdrücklich auch beim Nachziehen des Skills selbst. Sie wird geschrieben, BEVOR die nächste Handlung beginnt (Tool-Aufruf, HTTP-Aufruf, Datei, Subagent, Phasenwechsel): erst die Zeile, dann handeln — nie danach und nie gebündelt am Ende. Das erste Wort ist das laufende Kommando (`Work`, `Auto`, `Review`, `Fix`, `Plan`, `Settings`, `Update`); ruft eines ein anderes auf, stehen beide da (`Auto › Work`). Dann folgt die laufende Phase mit dem gerechneten Fortschritt darin — Zähler durch echten Nenner, ohne Nenner keine Zahl. Sie steht als eine Zeile in `~/.claude/planstack-status-<session_id>.txt`; Task und PR-Nummer sind klickbar. Damit sie sich während eines laufenden Subagenten überhaupt aktualisiert, braucht der `statusLine`-Eintrag ein `refreshInterval` (5–10 s).',
