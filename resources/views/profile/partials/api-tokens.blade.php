<section id="api-tokens">
    <header>
        <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('profile.api_tokens') }}</h2>
    ...
    @if (session('api_token'))
        <div x-data="{ copied: false }" class="mt-6 rounded-md border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/30 p-4">
            <p class="text-sm font-medium text-green-800 dark:text-green-300">
                {{ __('profile.new_token_name_copy_it_now_it_won_t_be', ['name' => session('api_token_name')]) }}
            </p>
            <div class="mt-2 flex items-center gap-2">
                <code x-ref="tok" class="flex-1 overflow-x-auto whitespace-nowrap rounded bg-white dark:bg-gray-800 px-2 py-1 text-xs text-gray-800 dark:text-gray-100 ring-1 ring-gray-200 dark:ring-gray-700">{{ session('api_token') }}</code>
                <button type="button"
    ...
    @if (session('status') === 'api-token-revoked')
        <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">{{ __('profile.token_revoked') }}</p>
    @endif
    ...
    <div class="mt-6">
        @forelse ($user->tokens()->latest()->get() as $token)
            <div class="flex items-center justify-between border-b border-gray-100 dark:border-gray-700 py-2 last:border-0">
                <div class="min-w-0">
                    <p class="truncate text-sm font-medium text-gray-800 dark:text-gray-100">{{ $token->name }}</p>
                ...
                <form method="POST" action="{{ route('profile.tokens.destroy', $token->id) }}"
                    ...
                    @csrf
                    @method('DELETE')
                    <button class="text-xs text-red-500 dark:text-red-400 hover:underline">{{ __('profile.revoke') }}</button>
                </form>
            </div>
        @empty
            <p class="text-sm text-gray-400 dark:text-gray-500">{{ __('profile.no_tokens_yet') }}</p>
        @endforelse
    </div>
</section>
