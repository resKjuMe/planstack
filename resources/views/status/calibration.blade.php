...
<x-status-shell :project="$project" :active="$active" :bare="true">
    <div x-data="{
        ...
        <x-page-head :title="__('common.calibration')">
            ...
            <div class="space-y-4">
                <div>
                    <div class="mb-1 font-semibold text-gray-700">{{ __('status.metrics') }}</div>
                ...
                <div>
                    <div class="mb-1 font-semibold text-gray-700">{{ __('status.charts_table') }}</div>
                    <ul class="list-disc space-y-1 ps-4">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-lg bg-white p-4 ring-1 ring-gray-200">
                <div class="text-xs font-medium text-gray-400">{{ __('status.median_deviation') }}</div>
                @if ($kpis['medianLabel'])
            ...
            <div class="rounded-lg bg-white p-4 ring-1 ring-gray-200">
                <div class="text-xs font-medium text-gray-400">{{ __('status.velocity') }}</div>
                @if ($kpis['spPerDay'] !== null)
                    <div class="mt-1 text-3xl font-bold text-gray-900">{{ number_format($kpis['spPerDay'], 1, ',', '') }} <span class="text-lg font-semibold text-gray-500">{{ __('status.sp_day') }}</span></div>
                    <div class="mt-1 text-sm text-gray-500">@if ($kpis['daysPerSpLabel'])Ø {{ $kpis['daysPerSpLabel'] }} {{ __('status.per_sp') }} · @endif {{ __('status.claim_merge') }}</div>
                @else
                    <div class="mt-1 text-3xl font-bold text-gray-300">—</div>
                    <div class="mt-1 text-sm text-gray-500">{{ __('status.claim_merge') }}</div>
                @endif
            </div>

            <div class="rounded-lg bg-white p-4 ring-1 ring-gray-200">
                <div class="text-xs font-medium text-gray-400">{{ __('status.accuracy_25') }}</div>
            ...
            <div class="rounded-lg bg-white p-4 ring-1 ring-gray-200">
                <div class="text-xs font-medium text-gray-400">{{ __('status.data_basis') }}</div>
                ...
                <div class="mt-1 text-sm text-gray-500">{{ __('status.tasks_with_a_file_estimate') }}</div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <div class="rounded-lg bg-white p-5 ring-1 ring-gray-200">
                <h2 class="font-semibold text-gray-900">{{ __('status.estimated_vs_actual') }}</h2>
                <p class="text-xs text-gray-400">{{ __('status.files_per_task_diagonal_perfect_estimate') }}</p>
                ...
                @else
                    <p class="mt-6 text-sm text-gray-400">{{ __('status.no_tasks_with_a_file_estimate') }}</p>
                @endif
            </div>

            <div class="rounded-lg bg-white p-5 ring-1 ring-gray-200">
                <h2 class="font-semibold text-gray-900">{{ __('status.accuracy_by_sp') }}</h2>
                <p class="text-xs text-gray-400">{{ __('status.share_within_25') }}</p>
        ...
        <div x-show="tab !== 'grouped'" class="overflow-hidden rounded-lg bg-white ring-1 ring-gray-200">
                        ...
                        <tr class="border-b border-gray-50 last:border-0">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <template x-if="!row.hasEstimate">
                                        <span class="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-500">{{ __('status.no_estimate_2') }}</span>
                                    </template>
                                </div>
                            </td>
                            <td class="px-4 py-3">
            ...
            <p x-show="list.length === 0" class="px-4 py-6 text-center text-sm text-gray-400">{{ __('status.no_entries') }}</p>
        </div>
        ...
        <div x-show="tab === 'grouped'" x-cloak class="overflow-hidden rounded-lg bg-white ring-1 ring-gray-200">
                ...
                <tbody>
                    @foreach ($groups as $group)
                        <tr class="border-b border-gray-50 align-top last:border-0">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-gray-600">Ø {{ number_format($group['avgDuration'], 1, ',', '') }} {{ __('status.days') }}</td>
                            <td class="px-4 py-3">
            ...
            @if ($groups->isEmpty())
                <p class="px-4 py-6 text-center text-sm text-gray-400">{{ __('status.no_entries') }}</p>
            @endif
        </div>
    </div>
</x-status-shell>
