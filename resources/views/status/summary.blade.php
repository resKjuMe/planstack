<x-status-shell :project="$project" :active="$active" :bare="true">
    <div class="space-y-8">
        ...
        <x-page-head :title="__('common.summary')">
            <ul class="list-disc space-y-1 ps-4">
                <li><span class="font-medium">{{ __('common.summary') }}</span>: {{ __('status.overview_of_the_project_s_progress') }}</li>
                <li>{{ __('status.kpi_tiles_progress_completed_prs_story') }}</li>
                <li><span class="font-medium">{{ __('common.phases') }}</span>: {{ __('status.colored_status_bars_hover_shows_count') }}</li>
                <li><span class="font-medium">{{ __('status.pickable_prs') }}</span>: {{ __('status.tasks_you_can_start_right_now_the_first') }}</li>
            </ul>
        </x-page-head>
                    ...
                    @foreach ($pickable as $task)
                        <div class="bg-white rounded-lg p-4 {{ $loop->first ? 'ring-2 ring-indigo-500 shadow-md' : 'ring-1 ring-gray-100 shadow-sm' }}">
                            ...
                            <div class="mt-2 flex items-center gap-3 text-xs text-gray-500">
                                ...
                                <span>·</span>
                                <span>{{ $task->affected_files ?? '—' }} {{ __('status.files') }}</span>
                            </div>
