...
(function () {
    ...
    function evaluate() {
        ...
        if (!installed) { show('install'); return false; }        // Userscript läuft nicht
        if (cmp(installed, current) < 0) {                         // ältere Version
            var slot = host.querySelector('[data-installed]');
            if (slot) slot.textContent = 'v' + installed;
            show('update');
            return true;
        }
        host.hidden = true;                                        // läuft & aktuell → nichts
        return true;
    }

    // Sofort prüfen, auf das „ready"-Event des Userscripts hören und kurz nachfassen
    // (falls es minimal später lädt).
