...
<div class="rounded-lg border border-orange-300 dark:border-orange-700 bg-orange-50 dark:bg-orange-900/30 p-5">
    <div class="flex items-start gap-3">
        <span class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400">
        ...
        <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-start justify-between gap-2">
                <div class="min-w-0">
                    <p class="font-semibold text-orange-900 dark:text-orange-200">{{ $c->summary }}</p>
                    <p class="text-xs text-orange-700/80 dark:text-orange-300/80">{{ __('tasks.concern_by_name', ['name' => $c->creator?->name]) }}</p>
                </div>
                @can('update', $task)
                    <div class="flex items-center gap-3 text-sm">
                        <a href="{{ route('projects.tasks.concern.edit', [$project, $task]) }}" class="font-medium text-orange-800 dark:text-orange-300 hover:underline">{{ __('common.edit') }}</a>
                        <form method="POST" action="{{ route('projects.tasks.concern.destroy', [$project, $task]) }}"
                              onsubmit="return confirm('{{ __('tasks.remove_concern') }}');">
                            @csrf @method('DELETE')
                            <button class="font-medium text-red-600 dark:text-red-400 hover:underline">{{ __('common.remove') }}</button>
                        </form>
                    </div>
                @endcan
            </div>
                ...
                <div class="mt-3 grid gap-3 sm:grid-cols-2 text-sm">
                    @foreach ($details as $label => $value)
                        <div @class(['sm:col-span-2' => $label === 'tasks.blocker'])>
                            <dt class="text-xs font-semibold uppercase tracking-wide text-orange-700/70 dark:text-orange-300/70">{{ __($label) }}</dt>
                            <dd class="text-orange-900/90 dark:text-orange-200/90"><x-markdown :content="$value" /></dd>
                        </div>
                    @endforeach
                </div>
            @endif
            ...
            @if (count($decisions))
                <div class="mt-4">
                    <div class="mb-2 flex items-center justify-between gap-2">
                        <h4 class="text-xs font-semibold uppercase tracking-wide text-orange-700/70 dark:text-orange-300/70">{{ __('common.open_decisions') }}</h4>
                    ...
                    <ul class="space-y-2">
                        @foreach ($decisions as $d)
                            <li class="text-sm">
                                <span class="font-medium text-orange-900 dark:text-orange-200">{{ $d['question'] }}</span>
                                @if (count($d['options']))
                                    <ul class="ms-4 list-disc text-orange-800/80 dark:text-orange-300/80">
                ...
                <x-modal name="claude-decisions" maxWidth="lg" focusable>
                    <div class="p-6"
                        ...
                        <div class="mb-4 flex items-start gap-3">
                            ...
                            <div class="min-w-0">
                                <h3 class="font-semibold text-gray-900 dark:text-gray-100">{{ __('tasks.find_a_decision_with_claude') }}</h3>
                                <p class="truncate text-xs text-gray-500 dark:text-gray-400" x-text="summary"></p>
                            </div>
                        </div>
                        ...
                        <template x-if="!done">
                            <div>
                                ...
                                <div class="mb-5 h-1.5 w-full overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                                    <div class="h-full rounded-full bg-[#D97757] transition-all" :style="'width:' + ((step + 1) / total * 100) + '%'"></div>
                                </div>

                                <p class="mb-3 font-medium text-gray-900 dark:text-gray-100" x-text="current.question"></p>
                                ...
                                <div class="space-y-2">
                                    <template x-for="(opt, i) in current.options" :key="i">
                                        <button type="button" @click="choose(opt)"
                                                ...
                                                :class="answers[step] === opt ? 'border-[#D97757] bg-[#D97757]/5 text-gray-900 dark:text-gray-100 ring-1 ring-[#D97757]' : 'border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700/50'">
                                            <span class="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700 text-xs font-semibold text-gray-500 dark:text-gray-400" x-text="String.fromCharCode(65 + i)"></span>
                                ...
                                <div class="mt-3">
                                    <label class="mb-1 block text-xs text-gray-400 dark:text-gray-500" x-text="current.options.length ? '{{ __('tasks.or_your_own_answer') }}' : '{{ __('tasks.answer') }}'"></label>
                                    <input type="text" x-model="custom[step]" @input="answers[step] = undefined"
                                           placeholder="{{ __('tasks.enter_your_own_decision') }}"
                                           class="block w-full rounded-md border-gray-300 dark:border-gray-600 text-sm shadow-sm focus:border-[#D97757] focus:ring-[#D97757]">
                                </div>

                                <div class="mt-6 flex items-center justify-between">
                                    <button type="button" @click="prev()" x-show="step > 0" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">{{ __('tasks.back') }}</button>
                        ...
                        <template x-if="done">
                            <div>
                                <p class="mb-3 text-sm text-gray-500 dark:text-gray-400">{{ __('tasks.decisions_made_review_them_and_launch') }}</p>
                                <ol class="mb-5 space-y-3">
                                    <template x-for="(d, i) in decisions" :key="i">
                                        <li class="text-sm">
                                            <p class="font-medium text-gray-800 dark:text-gray-100" x-text="(i + 1) + '. ' + d.question"></p>
                                            <div class="ms-4 flex items-center justify-between gap-3">
                                                <span class="text-[#a8492e]" x-text="'→ ' + (value(i) || '{{ __('tasks.not_specified') }}')"></span>
                                                <button type="button" class="shrink-0 text-xs text-indigo-600 dark:text-indigo-400 hover:underline" @click="step = i">{{ __('tasks.change') }}</button>
                                            </div>
                                        </li>
                                    </template>
                                </ol>
                                <div class="flex items-center justify-between">
                                    <button type="button" @click="step = total - 1" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">{{ __('tasks.back') }}</button>
