<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">{{ __('projects.new_project') }}</h2>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                <form method="POST" action="{{ route('projects.store') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ __('projects.e_g_demo_letters_numbers_and') }}</p>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ __('projects.optional_format') }} <span class="font-mono">owner/repo</span> {{ __('projects.for_pr_linking_and_pr_status_sync') }}</p>
                        <x-input-error :messages="$errors->get('github_repo')" class="mt-2" />
                    </div>

                    <div class="flex items-center justify-end gap-3">
                        <a href="{{ route('projects.index') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">{{ __('common.cancel') }}</a>
