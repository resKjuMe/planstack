<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('common.organization') }}</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            @if ($organization)
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    {{-- Mitglieder --}}
                    <div class="mt-6">
                        <h4 class="mb-3 text-sm font-semibold text-gray-600">{{ __('common.members') }}</h4>
                                    ...
                                    <tr class="border-b last:border-0">
                                        <td class="py-2 font-medium text-gray-800">
                                            ...
                                            @if ($organization->isOwner($member))
                                                <span class="ms-1 text-xs text-amber-600">{{ __('organization.founder') }}</span>
                                            @endif
                                            @if ($member->id === $user->id)
                                                <span class="ms-1 text-xs text-gray-400">{{ __('organization.you') }}</span>
                                            @endif
                                        </td>
                    ...
                    <div class="mt-6 border-t pt-5">
                        ...
                        @if ($isOwner)
                            <form method="POST" action="{{ route('organization.destroy') }}"
                                ...
                                <span class="ms-2 text-xs text-gray-400">{{ __('organization.removes_the_organization_for_all') }}</span>
                            </form>
                        @else
                ...
                @if ($isOwner)
                <div class="bg-white rounded-lg shadow p-6">
                    <h3 class="mb-1 font-semibold text-gray-900">{{ __('organization.invite_members') }}</h3>
                    ...
                    <form method="POST" action="{{ route('organization.invite') }}">
                        ...
                        @if ($assignableTeams->isNotEmpty())
                            ...
                            <div class="mt-5 border-t pt-5">
                                ...
                                <p class="mt-1 text-xs text-gray-400">{{ __('organization.the_invited_person_will_be_added_to') }}</p>
            ...
            @else
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    {{-- Gründen --}}
                    <div class="bg-white rounded-lg shadow p-6">
                        <h3 class="mb-1 font-semibold text-gray-900">{{ __('organization.create_organization') }}</h3>
                        <p class="mb-4 text-sm text-gray-500">{{ __('organization.create_a_new_organization_you') }}</p>
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <h3 class="mb-1 font-semibold text-gray-900">{{ __('organization.join_organization') }}</h3>
                        <p class="mb-4 text-sm text-gray-500">{{ __('organization.enter_the_personal_invitation_code_from') }}</p>
