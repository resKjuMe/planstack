...
Zusätzlich zeigt der Auto-Modus **dauerhaft** in einer **Sticky-Statuszeile**, was gerade läuft — Format `<Symbol> Auto (<Aktion>) <PROJECT> · <TASK> — <kurzer Schritt>` (z. B. `⚙ Auto (Review) DCE · A1 — Diff prüfen`), gespeist aus `~/.claude/planstack-status-<session_id>.txt` und damit **nur im laufenden Fenster** sichtbar.
