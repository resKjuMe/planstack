...
Zusätzlich zeigt der Auto-Modus **dauerhaft** in einer **Sticky-Statuszeile**, was gerade läuft — Format `<Symbol> Auto (<Phase> <%>) <PROJECT> · <TASK> — <kurzer Schritt>` (z. B. `⚙ Auto (Bearbeite 44 %) DCE · C27 — 4/9 Dateien`; die Prozentzahl ist der **gerechnete** Fortschritt innerhalb der Phase — Zähler durch echten Nenner, ohne Nenner keine Zahl), gespeist aus `~/.claude/planstack-status-<session_id>.txt` und damit **nur im laufenden Fenster** sichtbar. Task und PR-Nummer sind als OSC-8-Hyperlinks klickbar (Ctrl/Cmd+Klick), und die Zeile wird **immer als erste Handlung eines Schritts** geschrieben — vor dem Tool-Aufruf, nie danach. Wichtig: im `statusLine`-Eintrag **`refreshInterval`** setzen (Empfehlung 5–10 s) — sonst friert die Zeile ein, während ein Auto-Run als Subagent arbeitet.
