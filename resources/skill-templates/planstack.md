...
Zusätzlich zeigt der Auto-Modus **dauerhaft** in einer **Sticky-Statuszeile**, was gerade läuft — Format `<Symbol> Auto (<Aktion>) <PROJECT> · <TASK> — <kurzer Schritt>` (z. B. `⚙ Auto (Review) DCE · A1 — Diff prüfen`), gespeist aus `~/.claude/planstack-status-<session_id>.txt` und damit **nur im laufenden Fenster** sichtbar. Task und PR-Nummer sind als OSC-8-Hyperlinks klickbar (Ctrl/Cmd+Klick), und die Zeile wird **immer so früh wie möglich** aktualisiert — vor dem Schritt, den sie ankündigt, nicht danach. Wichtig: im `statusLine`-Eintrag **`refreshInterval`** setzen (Empfehlung 5–10 s) — sonst friert die Zeile ein, während ein Auto-Run als Subagent arbeitet.
